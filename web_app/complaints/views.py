...
def upload(request):
    if request.method == 'POST':
        description = request.POST.get('issue-desc')
        city_id = request.POST.get('select-city')
        district_id= request.POST.get('select-district')
        images = request.FILES.getlist('image')

        if len(description) < 10:
            error = 'Please the description should be at least 10 characters'
            return render(request, 'complaints.html', context={'error': error})
        else:
            if len(images) == 0:
